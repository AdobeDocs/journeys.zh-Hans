...
* 用于管理历程体验增长的新护栏：
   * 我们建议您将节点数量限制为 50 个或以下，以保证历程的性能、易读性、QA 和故障排除功能的流畅运行。活动数显示在历程画布的左上角部分。请参阅 Journey Optimizer [文档](https://experienceleague.adobe.com/docs/journey-optimizer/using/get-started/guardrails.html?lang=zh-Hans#journeys-guardrails-journeys)
   * 当您开发和发布历程时，我们会在接近达成 100 个实时历程的里程碑时向您发送通知。如果计划一次发布 100 个以上的历程，请在收到通知后创建支持工单，我们将为您提供帮助。请参阅 Journey Optimizer [文档](https://experienceleague.adobe.com/docs/journey-optimizer/using/get-started/guardrails.html?lang=zh-Hans#journeys-guardrails-journeys)
...
* 测试模式中添加了护栏，以仅侦听通过界面发送的事件。不会侦听通过外部工具发送的事件。请参阅 Journey Optimizer [文档](https://experienceleague.adobe.com/docs/journey-optimizer/using/orchestrate-journeys/create-journey/testing-the-journey.html?lang=zh-Hans)。

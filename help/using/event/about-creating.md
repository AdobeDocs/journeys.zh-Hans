...
1. 在字 **[!UICONTROL Event ID type]** 段中，选择要使用的事件类型。

   ![](../assets/journey6bis.png)

   * **基于规则的事件** :此类型的事件不生成eventID。 在事件 **ID条件字段** ，您只需定义一个规则，系统将使用该规则来标识将触发您的旅程的相关事件。 此规则可以基于事件有效负荷中可用的任何字段，例如用户档案的位置或添加到用户档案购物车的项目数。

   * **系统生成的** 事件:此类型需要eventID。 创建事件时会自动生成此eventID字段，并将其添加到有效负荷预览。 推送事件的系统不应生成ID，它应传递有效负荷预览中可用的ID。 请参阅[此章节](../event/previewing-the-payload.md)。
   >[!NOTE]
   >
   >阅读本节中有关事件类型 [的更多信息](../event/about-events.md)。
...
1. 定义架构和有效负载字段：在这里，您可以选择 [!DNL Journey Orchestration] 预期接收的事件信息（通常称为有效负载）。然后，您便能够在旅程中使用这些信息。请参阅[此页](../event/defining-the-payload-fields.md)。
   >[!NOTE]
   >
   >选择类型 **[!UICONTROL System Generated]** 时，只有具有eventID类型混合的模式可用。 选择类型时， **[!UICONTROL Rule Based]** 所有体验事件模式均可用。

1. 对于基于规则的事件，单击字 **[!UICONTROL Event ID condition]** 段内。 使用简单的表达式编辑器，定义系统将使用的条件，以识别将触发您旅程的事件。
